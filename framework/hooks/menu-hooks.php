...
class NavWalker extends \Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= "\n$indent<div class='dropdown-menu parent'> <ul role=\"menu\" class=\"dropdown-inner\">\n";
	}
	/**
	 * @see Walker::start_el()
	 * @since 3.0.0
	...
	public function display_element( $element, &$children_elements, $max_depth, $depth, $args, &$output ) {
        ...
        if (!empty($children_elements[$element->$id_field])) {
            foreach ($children_elements[$element->$id_field] as $child) {
                if ($child->current || $child->current_item_ancestor) {
        ...
        if ($element->current || $has_active_child) {
