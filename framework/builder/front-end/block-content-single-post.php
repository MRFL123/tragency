...
<section class="single-post ">
    ...
    <div class="container post-container">
        <div class="row p-4 bg-white  flex-column align-items-center">
            <?php if($image) : ?>
                <img src="<?= ($image['url']) ?>" alt="<?= ($image['alt']) ?>" class="post-image">
            <?php endif; ?>
            <h2 class="mt-4 font-30 text-black"><?= get_the_title() ?></h2>
            <span class="d-flex align-items-center mb-3">
                ...
                <p class="mb-0 ms-2 font-15 text-dark"><?= get_the_date() ?></p>
            </span>
            <?php if($post_text) : ?>
            ...
            <span class=" align-items-center">
                <p class="mb-0 me-3"><?= __('Share on' , 'fkgroup') ?></p>
                <?php if(have_rows('icons')) : ?>
                    <?php while(have_rows('icons')) : the_row();
                    $icon = get_sub_field('icon');
                    ?>
                    <img src="<?= ($icon['url']) ?>" alt="<?= ($icon['alt']) ?>" class="social-icon">
                    <?php endwhile ?>
                <?php endif; ?>
            </span>
        </div>
